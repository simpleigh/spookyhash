 ...
 * This is a derivative work based on the original SpookyHash tests created by
 * Bob Jenkins.
 * @see http://burtleburtle.net/bob/c/TestSpookyV2.cpp
 *
 * @author Leigh Simpson <code@simpleigh.com>
 * @license MIT
 * @copyright Original work Copyright Bob Jenkins.
...
describe('SpookyHash functions', () => {
    ...
    it('handles alignment correctly', () => {
        /**
         * Creates a test buffer with a simple message
         * @param {number} messageLength  length of message to create
         * @param {number} offset         bytes to offset start of the message
         * @returns {void}
         */
        const createBuffer = (messageLength, offset = 0) => {
            const buffer = Buffer.allocUnsafe(messageLength + offset);

            for (let index = 0; index < messageLength; index += 1) {
                buffer.writeUInt8(index % 256, index + offset);
            }
        ...
        /**
         * Assembles hashes of a message
         * @param {Buffer} message  message to hash
         * @returns {[number, BigInt, Buffer]} hash results
         */
        ...
        for (let messageLength = 0; messageLength < 1024; messageLength += 1) {
            const rawMessage = createBuffer(messageLength);
            const rawMessageHashes = computeHashes(rawMessage);

            for (let offset = 1; offset < 8; offset += 1) {
                const offsetMessage = createBuffer(messageLength, offset);
                expect(computeHashes(offsetMessage.slice(offset)))
                    .toStrictEqual(rawMessageHashes);
            }
        }
    });
